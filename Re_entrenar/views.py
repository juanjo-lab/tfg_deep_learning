    ...
    if request.user.is_anonymous(): # si no tiene acceso el usuario, lo bloqueara
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                newdoc.save()
                Numero = len(glob.glob(str(os.environ['DIR_PER']) + "/Crear_Red/" +str(request.user.username) + "/*.csv"))
...
def send__email(mail,username):
    ...
    #adjuntar archivos
    filename =str(os.environ['DIR_PER']) + "/Re_entrenar/"+username+"/Re_entrenar_"+username+".jpg"
...
# -----------------------------------------------------------------------------------------------------------------------
    ...
    initial_path = str(os.environ['DIR_PER']) + '/Re_entrenar/'+Patient+'/'
    initial_path_cp = str(os.environ['DIR_PER']) + '/Predecir/'+Patient+'/'
    ...
    num_epochs = os.environ['EPOCH_R']
    # Tamaño del lot
    training_batch_size = os.environ['BATCH_SIZE']
    ...
    opt = Adam(learning_rate=os.environ['LEARNING_RATE_R'], beta_1=0.9)
    ...
    plt.savefig(str(os.environ['DIR_PER']) + "/Re_entrenar/"+Patient+"/Perdidas_" + Patient + ".jpg",bbox_inches="tight")
...
###############################################################################################################################
    ...
    plt.legend()
    plt.savefig(str(os.environ['DIR_PER']) + "/Re_entrenar/" + Patient + "/Re_entrenar_" + Patient + ".jpg",bbox_inches="tight")
    ########################################################################################################################
