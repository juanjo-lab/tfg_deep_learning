...
def LSTM_CREAR(request,Patient):
    ...
    model.add(LSTM(units=os.environ['UNITS_'], return_sequences=True, input_shape=(features_set.shape[1], 1)))
    model.add(Dropout(os.environ['DROPOUT']))
    ...
    model.add(LSTM(units=os.environ['UNITS_'], return_sequences=True))
    model.add(Dropout(os.environ['DROPOUT']))
    model.add(LSTM(units=os.environ['UNITS_'], return_sequences=True))
    model.add(Dropout(os.environ['DROPOUT']))
    model.add(LSTM(units=os.environ['UNITS_']))
    model.add(Dropout(os.environ['DROPOUT']))
