    ...
    if request.user.is_anonymous():
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                FC = request.POST['name_CF']

                print(FC)

                BOLO_12h,BOLO_24h,CF =Predict(request,request.user.username,FC)
...
########## test
    ...
    CF, BOLO_24h = correccion(unidades,np.max(predictions[1:24]))
    ...
    ####################################
    ################################# 12h
    CF, BOLO_12h = correccion(unidades, np.max(predictions[1:12]))
